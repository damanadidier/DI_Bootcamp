...
# Initialize an empty dictionary to store the letter indices
letter_indices = {}
...
for item, price in items_purchase.items():
    # Clean the price and convert it to an integer
    item_price = int(price.replace("$", "").replace(",", ""))
    # Check if the item can be afforded with the current wallet amount
    if item_price <= wallet_amount:
        # If it can be afforded, add it to the basket and update the wallet amount
        basket.append(item)
        wallet_amount -= item_price
# Check if the basket is empty and print the appropriate output
if not basket:
    print("Nothing")
else:
    print(sorted(basket))
